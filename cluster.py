...
class Cluster():
    ...
    def load_machines(self, csv_file_name):
        ...
        for line in lines:
            elements = line.split(",")
            # exclude the last element (cpu frequency)
            self.machines.append(Machine(elements[0], *map(int, elements[1:4])))
    ...
    def best_bin_first(self, job, global_clock):
        ...
        print(f"{job_to_stop} not found running on any machine")
